...
class EmulatorClient:
    ...
    def get_team(self):
        ...
        if game_state:
            return game_state.split("Pokemon Party")[1]
    ...
    def press_buttons(
        ...
        response = requests.post(
            f"{self.base_url}/api/press_buttons", json=data)
        if response.status_code != 200:
            logger.error(
    ...
    def navigate(self, row, col, include_state=False, include_screenshot=False):
        ...
        if response.status_code != 200:
            logger.error(
                f"Error navigating: {response.status_code} - {response.text}")
    ...
    def initialize(self, max_retries=5, retry_delay=3):
        ...
        for attempt in range(1, max_retries + 1):
            try:
                logger.info(
                    f"Checking server status (attempt {attempt}/{max_retries})")
...
class PokemonAgent:
    def __init__(
        ...
        """
        self.client = EmulatorClient(
            host=server_host, port=server_port or 9876)
        ...
        logger.debug(
            f"Agent initialized with display config: {self.display_config}")
...
The summary should be comprehensive enough that you can continue gameplay without losing important context about what has happened so far."""
    ...
    def process_tool_call(self, tool_call):
        ...
        if tool_name == "press_buttons":
            ...
            content = [
                {"type": "text",
                    "text": f"Pressed buttons: {', '.join(buttons)}"},
                {
                    "type": "text",
    ...
    # Implement the logic to get team information
            team_info = self.client.get_team()
            ...
            return {
                ...
                "content": [
                    {"type": "text",
                        "text": f"Pokemon Team ({detail_level}):\n{team_info}"}
                ],
            }
        else:
    ...
    def run(self, num_steps=1, instance_id=None, snapshot_name_prefix=None):
        ...
        while self.running and steps_completed < num_steps:
            try:
                ...
                if self.display_config["quiet_mode"]:
                    logger.debug(
                        f"Completed step {steps_completed}/{num_steps}")
                else:
                    logger.info(
                        f"Completed step {steps_completed}/{num_steps}")
                ...
                if self.morph_client and instance_id:
                    ...
                    try:
                        ...
                        logger.info(
                            f"✅ Snapshot created with ID: {snapshot.id}")
                        logger.info(
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Run a Pokemon Game Server Agent")
    parser.add_argument(
...
def main():
    ...
    console.print(
        f"Parent snapshot ID for lineage tracking: {parent_snapshot_id}")
